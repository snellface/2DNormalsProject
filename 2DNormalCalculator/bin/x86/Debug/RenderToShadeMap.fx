			...
			float2 vPos : VPOS)
{
	...
	float4 colorMap = tex2D(ColorMapSampler, texCoord);
	...
	float pixelElevation = topmostLayer.g * 255;
...
// Normals prep	
		surfaceNormal = float3(normalMap.xyz * 2 - 1);
		surfaceNormal.y = -surfaceNormal.y;
		surfaceNormal = normalize(surfaceNormal);

		lightToPixel = float3(lightToPixel.xy, pixelElevation - LightPos.z);
...
// DIFFUSE LIGHT
		...
		if(lambertTerm > 0)
		{
			...
			color += float4(LightColor, 1) * LightIntensity * lambertTerm / lightDistance;
			color *= colorMap;
//color = float4 (lightToPixel / 2 + 0.5, 1);
		}
		else
		{
			//discard;
...
// SPECULAR LIGHT
		if(topmostLayer.b > 0)
		{
			float3 pixelToEye = normalize( float3(vPos, pixelElevation) - EyeLocation );
			...
			{
				float specularIllumination = pow(abs(reflectionDotProduct), 20) * topmostLayer.b;
...
// copy end
	...
	//color = float4(lambertTerm,lambertTerm,lambertTerm ,1);
	//color = float4(lightToPixel, 1);
